...
function TransferForm() {
  ...
  return (
    <div className="form-wrap">
      <img
        id="big-coin"
        src={require("../assets/images/coin-spin-300.gif")}
        alt=""
      />
      <form>
        <label for="recipient">Recipient:</label>
        <input id="recipient" placeholder="THE MOVEMENT" type="text" disabled />
        <SligoilTickWhite />
        <br />
        <label for="currency">Currency:</label>
        <input id="currency" placeholder="BIRDCOIN" type="text" disabled />
        <SligoilTickWhite />
        <br />
        ...
        />
        <SligoilTickWhite visible={isCorrect ? "visible" : "hidden"} />
        <br />
        {/* Tick only appears when correct key is input */}
        {isCorrect == false && (
          <p className="bad-key">You have entered an incorrect key.</p>
        )}
        <br />
